...
{
public:
	void set_end_points(std::string video_name, SOCKET sock);
	ScenarioCreator();
	void update();
	~ScenarioCreator();
...
private:
	...
	void spawn_peds_detect(Vector3 spawnAreaCenter, int numPed);   // pedestrians for evalte detection
	void spawn_peds1(Vector3 spawnAreaCenter, int numPed);		// function used to spawn pedestrians at the beginning of the scenario
