...
class SimpleNet(torch.nn.Module):
    def __init__(self, cfg):
        ...
        for i in range(self.cfg['LAYER_NUMBER']):
            self.hidden.extend([
                torch.nn.Sequential(
                    torch.nn.Conv2d(OUT_CHANNEL, OUT_CHANNEL, KERNEL_SIZE, dilation=DILATION, padding=PADDING),
                    torch.nn.BatchNorm2d(OUT_CHANNEL),
                )
            ])
            self.relu.extend([torch.nn.Conv2d(OUT_CHANNEL, OUT_CHANNEL, KERNEL_SIZE, dilation=DILATION, padding=PADDING)])
    ...
    def forward(self, x, img_h, img_w):
        ...
        for i in range(self.cfg['LAYER_NUMBER'] - 3):
            mask = self.relu[i](x)
            mask[mask < torch.mean(mask)] = 0  # 这其实没啥意义啊，这句话和relu有啥区别呢。。。
            x = self.hidden[i](x) * mask
