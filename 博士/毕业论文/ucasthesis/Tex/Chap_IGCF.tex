...
为了利用两个结果的优点并克服它们的缺点，我们提出了实例引导的自纠正（instance guided self-correction，IGSC）组件。IGSC 结合了实例分割和相关滤波结果，以提高跟踪的鲁棒形。具体而言，利用 $m$ 获得目标的几何中心：
\begin{equation}
c_{m} = Centroid(m),
\end{equation}
...
\tilde{w}_d^{lrn} = max(f_d \star h_d).
\end{equation}
...
\begin{figure}[t]
    ...
    \caption{预期平均重叠（EAO）图。跟踪器根据 VOT2015 \cite{Kristan2015TheVO} 的预期平均重叠值从右到左排列。最右边的跟踪器是跟踪性能最好的跟踪器。}
    \label{fig:vot15}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{预期平均重叠（EAO）图。跟踪器根据 VOT2016 \cite{Kristan2016TheVO} 的预期平均重叠值从右到左排列。最右边的跟踪器是跟踪性能最好的跟踪器。}
    \label{fig:vot16}
\end{figure}
\begin{equation}
