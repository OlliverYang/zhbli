...
COCO2017 \cite{COCO} 实例分割数据集用于训练InstMask。
...
InstMask允许学习更好的滤波器，并有助于纠正不良的跟踪结果。
...
配备了InstMask模块，所提出的跟踪器不仅可以很好地执行目标跟踪，还可以用于视频分割。DAVIS \cite{Perazzi2016} 是一个视频目标分割数据集，它由五十个高质量视频序列组成，跨越了多次出现的常见视频目标分割挑战，例如遮挡，运动模糊和外观变化。我们在DAVIS2016的验证集上进行了定性实验。可视化结果如图 \ref{fig:davis}所示。结果表明，提出的IGCF可以准确地恢复目标的轮廓，这在一定程度上解释了IGCF为什么可以提高跟踪性能。
