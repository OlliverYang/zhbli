...
\textbf{消融实验}
为了分析本章提出的时间聚合模块和对抗性 Dropout 模块的有效性，我们训练了两个跟踪网络的变体。变体 1（参见表 \ref{table:end_ablation} 第一行）在我们最终提出的跟踪器中同时剔除了时间聚合模块和对抗性 Dropout 模块。变体 2（参见表 \ref{table:end_ablition} 第二行）在我们最终提出的跟踪器中剔除对抗性 Dropout 模块，仅保留时间聚合模块。表 \ref{table:end_abliation} 的第三行表示我们最终提出的跟踪器，同时配备有时间聚合模块和对抗性 Dropout 模块。
