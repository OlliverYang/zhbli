...
\noindent We thank the editor-in-chief for giving us the oppotunity to resubmit the revised manuscript. We also thank the reviewers for their constructive comments on the manuscript and we explain their concerns point by point.
...
Similar to the perturbation on the template image, we sacrifice some imperceptibility of the adversarial patch on the search image to improve the attack efficiency, as shown in Table. \ref{tab:iter}.
...
Thanks for your advice and we have added experiments as you suggested. Specifically, we compare with state-of-the-art attack methods including RTAA, SPARK, TTP, CSA and FAN in both targeted and untargeted attack settings in Sec. IV. F, as shown in Table \ref{tab:untargeted} and Table \ref{tab:targeted}. We do not compare 
\begin{table}[h]
    \centering
...
When the ground truth box information of videos is not available, we can run the tracker (that needs to be attacked) on videos, generating one boxe per frame. The predicted boxes can be considered as ground truth for training our attack method. Specifically, we use GOT-10k training split as
\\[6pt]
