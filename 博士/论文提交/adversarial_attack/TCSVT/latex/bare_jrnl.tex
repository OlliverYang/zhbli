...
\textit{White box attacks v.s. Black box attacks} In the white box attack setting, the adversary has full knowledge of the model including model type, model architecture and values of all parameters and trainable weights. In the black box setting, the adversary has limited or no knowledge about the model under attack \cite{kurakin2018adversarial}. In this paper, we focus on the white box attack for siamese trackers. However, although we train perturbations using the parameters of the SiamFC++ network, we surprisely find that the trained perturbations can perform black attack on other trackers such as SiamRPN and SiamRPN++.

\textit{Non-universal attacks v.s. Universal attacks} In the setting of non-universal attacks, the adversary has to generate one perturbation of every new datapoint, which is time consuming. In the setting of the universal attacks, only one perturbation is generated and used for every datapoint of the dataset. In this paper, we focus on generating universal perturbations for siamese trackers to perform efficient attack.
...
It's important to make the size of the adversarial patch small to avoid suspicion. In our experiment, we produce three different sizes of the adversarial patch, namely, 32-by-32, 16-by-16 and 8-by-8 to test the efficiency of their attacks.
As shown in Table, the 16-by-16 patch can achieve ? and its area only accounts for ?\% of the searched image area.

\begin{figure}[t!]
  \begin{center}
    \includegraphics[width=0.48\textwidth]{images/patch_size/patch_size.png}
  \end{center}
  \caption{Influence of patch size.}
  \label{fig:patch_size_table}
\end{figure}

\begin{figure}[t!]
  \begin{center}
    \includegraphics[width=0.15\textwidth]{images/patch_size/32.jpg}
    \includegraphics[width=0.15\textwidth]{images/patch_size/16.jpg}
    \includegraphics[width=0.15\textwidth]{images/patch_size/8.jpg}
  \end{center}
  \caption{Influence of patch size.}
...
We further analyze the perfor-mance of our method with respect to 11 attributes in OTB-2015 [22]. As suggested in OTB [22], the attributes consistof out-of-view (OV), occlusion (OCC), illumination varia-tion (IV), out-of-plane rotation (OPR), scale variation (SV),deformation (DEF), low resolution (LR), fast motion (FM),background clutters (BC), motion blur (MB), and in-planerotation (IPR).
