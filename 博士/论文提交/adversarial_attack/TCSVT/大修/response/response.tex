...
% 解释paste（说明不需要加 constraint）
If we paste the perturbation to the image, we means we use a patch to replace the original image, which means the patch and the original image is discrete. The final pixel value is changed from the original value to a new value.
As a result, even though the perturbation is small, when we paste it to the image, it shows nearly black but not imperceptible.
% 解释加法
...
\uline{
If we paste the perturbation to the image, we means we use a patch to replace the original image, which means the patch and the original image is discret. Even though the perturbation is small, when we paste it to the image, it shows black but not imperceptible.
% 解释加法
