...
% 我们的工作和数字水印类似，都是向视频帧中增加 slightly perturbed（modifying the pixel values。）。但是我们的目的不同。数字水印的目的是通过检测是否存在扰动，从而判断该视频是否是正版视频，从而进行版权保护，而我们通过添加扰动来降低跟踪效果。
\cite{9343885}

% 人脸欺诈指的是使用假脸希望得到错误身份。我们的工作和人脸欺诈类似，都希望欺骗机器学习系统。然而人脸欺诈在真实环境中用假脸欺骗算法（物理攻击），我们是数字攻击，直接修改网络的输入。
Face presentation attacks \cite{9303455}: an imposter tries to be authenticated as a genuine client by presenting fake

% 图像分类的白盒攻击和防御，而我们针对的是跟踪的攻击，更加复杂，对实时性有更高要求。
dsfds \cite{9169672} fds
