...
Recently, the robustness of Siamese trackers have recently attracted a lot of attention \cite{TemporallyTransferablePerturbations,efficient,SPARK}. Despite the impressive performance of Siamese architectures on challenging visual object tracking task, these trackers were shown to be highly vulnerable to perturbations.
\cite{TemporallyTransferablePerturbations} propose a framework to generate a single temporally transferable adversarial perturbation from the object template image only. This perturbation can then be added to every search image to adversarially attack the subsequent video frames.
\cite{efficient} present an end-to-end network FAN (Fast Attack Network) that uses a novel drift loss combined with the embedded feature loss to attack the Siamese network based trackers.
\cite{SPARK} propose the spatial-aware online incremental attack (a.k.a. SPARK) that performs spatial-temporal sparse incremental perturbations online and makes the adversarial attack less perceptible.

All the above methods can fool object trackers successfully. However, these attack methods are not suitable for the real-world online-tracking phase where we can not get access to the limited computational resources.
The reason is, visual object tracking is a computationally intensive task which usually needs to be run in real time, but the computational resources of small platforms (e.g., mobile, robotic, self-driving car) used to deploy the trackers are often limited.
Therefore, the attack approaches for object tracking should occupy as little computing resources as possible.
But the reality is that most attack algorithms either perform iterative optimisation to calculate adversarial examples, or generate perturbations through Generative Adversarial Networks which occupy the GPU memory.

Universal adversarial perturbation (UAP) \cite{UAP} is an image-agnostic perturbation which can fool most images from a data distribution. Being universal, UAPs can be conveniently exploited to perturb unseen datapoints on-the-fly without extra computation. Therefore, UAPs are particularly useful when attacking real-time applications deployed on platforms with limited computing resources.
However, no existing work has touched the topic of attacking the Siamese trackers using UAPs, because it is difficult to apply existing UAPs in Siamese trackers directly. The reasons come from two aspects.
\begin{itemize}
\item Different input. Most UAPs are designed for typical neural networks, which usually accept one image as input; while Siamese-based tracking networks accept two inputs: the template image and the search image.
\item Different goals. The goal of existing UAP methods is to disturb unary and binary model outputs for single instance; while we need to use universal perturbations to mislead Siamese trackers along with a specified trajectory.
\end{itemize}

In this paper, we make the first attempt in finding the universal perturbations that fool a state-of-the-art deep Siamese trackers on all natural videos. We show in this paper the existence of such universal perturbations that cause the tracker to track object along any specified trajectory.
...
Different from the existing attack methods which are intrinsic dependent on videos, we generate the perturbations that can fool the network on most natural videos. Perturbing a new target then only involves the mere addition/paste operation of the universal perturbations to the template/search image -- and does not require solving an optimisation problem or network inference.
