...
Formally, we aim to train an imperceptible perturbation $\delta$ for the template image $\textbf z$, and an adversarial patch $p$ for the search image $\textbf x$. After adding $\delta$ to $\textbf z$ and pasting the fake target $p$ to $\textbf x$, the tracker outputs the location of the adversarial patch instead of the real target (see Figure \ref{fig:net}).
...
\label{alg:algorithm}
\textbf{Input}: Training dataset $\mathcal{V}$, Siamese tracker $T$, max iteration $N$\\
\textbf{Output}: Universal perturbation $\delta$ and universal patch $p$.
\begin{algorithmic}[1] %[1] enables line numbers
\STATE Let $i = 0$.
\WHILE{$i < N$}
\STATE Random pick a video $V\in \mathcal{V}$. The corresponding ground truth is $B^{gt}=\{b^{gt}_i\}^T_i$.
\STATE Random pick two frames $I_t, I_s$ from $v$.
\STATE Generate template image $\textbf{z}$ according to $I_t$ and $b^{gt}_t$.
\STATE $\tilde z = z + \delta$
...
\STATE $\tilde{\textbf x} = A(\textbf x, p_i, (l^x, l^y), (w, h)).$
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.93\textwidth]{images/vis_v3.pdf}
...
As mentioned before, the patch is regarded as a fake target and pasted on the search image. We force the center position of the fake target to near the center position of the real target (i.e., the center of the search image) with a shift of 64 pixels, where shift is defined as the max range of translation generated by a uniform distribution.
%an offset $(\Delta_x, \Delta_y)$.
%We set this offset to $(\pm 64, \pm 64)$. 
%$\Delta_x$ (as well as $\Delta_y$) is randomly selected between -64 pixels and 64 pixels.
